...
def validate_contact(contact: str) -> bool:
    if not contact.startswith("628") or len(contact) > 14:
        print(f"{bcolors.FAIL}Invalid number{bcolors.ENDC}")
        return False
    return True
...
def get_otp(contact: str) -> str:
    ...
    print(f"{bcolors.OKCYAN}Requesting OTP...{bcolors.ENDC}")
    try:
        ...
        if "subscriber_id" not in json_body:
            print(f"{bcolors.FAIL}{json_body.get('error', 'No error message in response')}{bcolors.ENDC}")
            raise ValueError("Subscriber ID not found in response")
        
        return json_body["subscriber_id"]
    except Exception as e:
        print(f"{bcolors.FAIL}Error requesting OTP: {e}{bcolors.ENDC}")
        return None
...
def extend_session(subscriber_id: str) -> str:
    ...
    print(f"{bcolors.OKCYAN}Extending session...{bcolors.ENDC}")
    try:
        response = requests.get(url, headers=headers, params=querystring, timeout=30)
        if response.status_code != 200:
            print(f"{bcolors.FAIL}Failed to extend session: {response.status_code} - {response.text}{bcolors.ENDC}")
            return None
        ...
        print(f"{bcolors.FAIL}Error extending session: {e}{bcolors.ENDC}")
        return None
...
def submit_otp(
    ...
    if contact_type == "SMS":
        ...
        if not code or len(code) != 6:
            print(f"{bcolors.FAIL}Invalid OTP code format{bcolors.ENDC}")
            return None
        final_code = code
        ...
        print(f"{bcolors.FAIL}Unsupported contact type{bcolors.ENDC}")
        return None
    ...
    print(f"{bcolors.OKCYAN}Submitting OTP...{bcolors.ENDC}")
    try:
        response = requests.post(url, data=payload, headers=headers, timeout=30)
        json_body = json.loads(response.text)
                
        if "error" in json_body:
            print(f"{bcolors.FAIL}[Error submit_otp]: {json_body}{bcolors.ENDC}")
            return None
        
        print(f"{bcolors.OKGREEN}Login successful.{bcolors.ENDC}")
        return json_body
    except requests.RequestException as e:
        print(f"{bcolors.FAIL}[Error submit_otp]: {e}{bcolors.ENDC}")
        return None
...
def get_new_token(api_key: str, refresh_token: str, subscriber_id: str) -> str:
    ...
    print(f"{bcolors.OKCYAN}Refreshing token...{bcolors.ENDC}")
    resp = requests.post(url, headers=headers, data=data, timeout=30)
    if resp.status_code == 400:
        if resp.json().get("error_description") != "Session not active":
            print(f"{bcolors.FAIL}Failed to refresh token: {resp.status_code} - {resp.text}{bcolors.ENDC}")
            return None
...
def get_auth_code(tokens: dict, pin: str, msisdn: str):
    ...
    try:
        ...
        print(f"{bcolors.FAIL}[get_auth_code] Request error: {e}{bcolors.ENDC}")
        return None


    if resp.status_code != 200:
        print(f"{bcolors.FAIL}Failed to get auth code: {resp.status_code} - {resp.text}{bcolors.ENDC}")
        return None

    try:
        data = resp.json()
    except ValueError:
        print(f"{bcolors.FAIL}Invalid JSON response: {resp.text}{bcolors.ENDC}")
        return None

    if not isinstance(data, dict):
        print(f"{bcolors.FAIL}Unexpected response format: {data!r}{bcolors.ENDC}")
        return None
    
    status = data.get("status", "")
    if status != "Success":
        print(f"{bcolors.FAIL}Error getting authorization code: {status}{bcolors.ENDC}")
        return None

    authorization_code = data.get("data", {}).get("authorization_code")
    if not authorization_code:
        print(f"{bcolors.FAIL}Authorization code not found in response: {data}{bcolors.ENDC}")
        return None
